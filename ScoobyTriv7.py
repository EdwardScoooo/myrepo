...
##this is for the plot, to adjust sizing etc
planeAxis = [-30, 30, -30, 30]

picSize=[-1.7,16.7,-1.7,2.3]
titleF= {'family':'serif','color':'red','size':17}
...
def calculateXandYcase1(phi, theta, Fdis, units):
  ...
  if signFlag == 0:
    ...
    im = plt.imread("scoobies.jpg")

    fig,ax=plt.subplots()

    im=ax.imshow(im,extent=picSize)
    ...
    plt.title('GUNSHOT TRIANGULATION', fontdict=titleF)
    plt.xlabel(units)
    plt.ylabel(units)
